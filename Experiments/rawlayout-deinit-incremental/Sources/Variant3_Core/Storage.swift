// MARK: - Variant 3: Core Module with Full SwiftSettings
// Testing if SwiftSettings (strictMemorySafety, InternalImportsByDefault, etc.) cause the issue
...
public enum Storage<Element: ~Copyable> {
    public struct Inline<let capacity: Int>: ~Copyable {
        ...
        deinit {
            print("Variant3 Storage.Inline deinit called (count=\(_count))")
            _tracker?.increment()
