...
struct StorageInlineTests {
    ...
    func `pointer returns correct address`() {
        ...
        let ptr: UnsafePointer<Int> = unsafe storage.pointer(at: 3 as Index<Int>.Bounded<8>)
    ...
    func `mutable pointer allows modification`() {
        ...
        let ptr: UnsafeMutablePointer<Int> = unsafe storage.pointer(at: 0 as Index<Int>.Bounded<8>)
    ...
    func `deinitialize range with noncopyable elements via pointer init`() {
        ...
        do {
            ...
            let ptr: UnsafeMutablePointer<TrackedValue> = unsafe storage.pointer(at: 0 as Index<TrackedValue>.Bounded<3>)
    ...
    func `explicit cleanup via deinitialize all clears bitvector tracked elements`() {
        ...
        final class Marker: @unchecked Sendable {
            ...
            init() { unsafe Self.instanceCount += 1 }
            deinit { unsafe Self.instanceCount -= 1 }
        }
