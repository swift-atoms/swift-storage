...
struct StorageInlineTrackedTests {
    ...
    // MARK: - slotCapacity Tests

    @Test
    func `slotCapacity matches compile-time capacity`() {
        let storage8 = Storage<Int>.Inline<8>()
        #expect(storage8.slotCapacity == Index<Int>.Count(8))

        let storage1 = Storage<Int>.Inline<1>()
        #expect(storage1.slotCapacity == Index<Int>.Count(1))

        let storage256 = Storage<Int>.Inline<256>()
        #expect(storage256.slotCapacity == Index<Int>.Count(256))
    }
    ...
    func `copy all to heap`() throws {
        ...
        let heap = Storage<Int>.Heap.create(minimumCapacity: Index<Int>.Count(8))
        inline.copy(to: heap)
