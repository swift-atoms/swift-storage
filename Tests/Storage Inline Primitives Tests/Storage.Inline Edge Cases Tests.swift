...
struct StorageInlineEdgeCaseTests {
    ...
    struct AlignmentExtremes {
        ...
        func `mixed alignment in sequence`() {
            ...
            let ptr0: UnsafeMutablePointer<MixedPadding> = unsafe storage.pointer(at: 0 as Index<MixedPadding>.Bounded<4>)
            let ptr1: UnsafeMutablePointer<MixedPadding> = unsafe storage.pointer(at: 1 as Index<MixedPadding>.Bounded<4>)
    ...
    struct RapidCycles {
        ...
        func `rapid initialize-move cycles all slots`() {
            ...
            for round in 0..<100 {
                ...
                for i: Index<Int>.Bounded<8> in [0, 1, 2, 3, 4, 5, 6, 7] {
                    storage.initialize(to: round * 8 + Int(bitPattern: Index<Int>(i)), at: i)
                }
                ...
                // Move all
                for i: Index<Int>.Bounded<8> in [0, 1, 2, 3, 4, 5, 6, 7] {
                    let expected = round * 8 + Int(bitPattern: Index<Int>(i))
        ...
        func `rapid initialize-deinitialize cycles with tracking`() {
            final class Tracker: @unchecked Sendable {
                ...
                init() {
                    unsafe Self.liveCount += 1
                    unsafe Self.totalCreated += 1
                }
                deinit { unsafe Self.liveCount -= 1 }
            }
    ...
    struct SparseInitialization {
        ...
        func `single element ranges at boundaries`() {
            final class Tracker: @unchecked Sendable {
                ...
                deinit { unsafe Self.deinitOrder.append(id) }
            }
    ...
    struct CrossStorageEdgeCases {
        ...
        func `move entire capacity to heap`() {
            ...
            // Fill completely
            for i: Index<Int>.Bounded<8> in [0, 1, 2, 3, 4, 5, 6, 7] {
                inline.initialize(to: Int(bitPattern: Index<Int>(i)) * 11, at: i)
            }
            ...
            for i: Index<Int> in [0, 1, 2, 3, 4, 5, 6, 7] {
                ...
                #expect(value == Int(bitPattern: i) * 11)
            }
        }
        ...
        func `copy with class elements shares object identity and preserves source`() {
            final class Tracker: @unchecked Sendable {
                ...
                init(_ id: Int) {
                    self.id = id
                    unsafe Self.instances += 1
                }
                deinit { unsafe Self.instances -= 1 }
            }
    ...
    struct PointerArithmeticStress {
        ...
        func `verify all pointers are within storage bounds`() {
            ...
            for i in 0..<16 {
                let ptr: UnsafeMutablePointer<Int64> = unsafe storage.pointer(at: Index<Int64>.Bounded<16>(Ordinal(UInt(i))))
        ...
        func `pointer modification affects correct slot only`() {
            ...
            let slot4: Index<Int>.Bounded<8> = 4
            let ptr: UnsafeMutablePointer<Int> = unsafe storage.pointer(at: slot4)
    ...
    struct MemoryLayoutVerification {
        ...
        func `bool storage is compact`() {
            ...
            #expect(
                boolStorageSize <= boolIdealSize + bitVectorSize + 16,
