...
extension Storage.Inline where Element: ~Copyable {
    ...
    @inlinable
    func linearIndex<let N: Int>(from cyclicIndex: Index<Element>.Cyclic<N>) -> Index<Element> {
    ...
    @inlinable
    public mutating func pointer<let N: Int>(at index: Index<Element>.Cyclic<N>) -> Pointer<Element> {
        pointer(at: linearIndex(from: index))
    }
    ...
    /// - Precondition: The element at `index` must be initialized.
    @inlinable
    public mutating func pointer<let N: Int>(at index: Index<Element>.Cyclic<N>) -> Pointer<Element>.Mutable {
        pointer(at: linearIndex(from: index))
    }
    ...
    @inlinable
    public func deinitialize<let N: Int>(header: Storage<Element>.Ring.Header.Cyclic<N>) {
        deinitialize(head: header.headIndex, count: header.count)
    }
}

// MARK: - Linearization with Cyclic Header

extension Storage.Inline where Element: ~Copyable {

    /// Moves elements to heap storage using a cyclic header.
    ///
    /// Elements are moved from ring buffer order to linear order starting at
    /// index 0 in the destination storage. Source slots are deinitialized.
    ///
    /// - Parameters:
    ///   - heapStorage: The destination heap storage.
    ///   - header: The cyclic ring buffer header.
    /// - Precondition: Elements from head through count positions must be initialized.
    /// - Precondition: Heap storage must have sufficient capacity.
    @inlinable
    public mutating func linearize<let N: Int>(
        to heapStorage: Storage<Element>,
        header: Storage<Element>.Ring.Header.Cyclic<N>
    ) {
        ...
        _ = unsafe withUnsafeMutablePointer(to: &_storage) { base in
            unsafe heapStorage.withUnsafeMutablePointerToElements { destination in
                ...
                (.zero..<header.count).forEach { dstIndex in
                    let linearIdx = Index<Element>(Ordinal(index.rawValue.position.rawValue))
                    let source: Pointer<Element>.Mutable = address.pointer(
                        at: linearIdx,
                        stride: Self.slotStride,
                        as: Element.self
                    )
                    unsafe (destination + dstIndex).initialize(to: source.move())
                    index += .one
                }
            }
        }
...
// MARK: - Copy with Cyclic Header (Copyable Elements)

extension Storage.Inline where Element: Copyable {

    /// Copies elements to heap storage using a cyclic header.
    ///
    /// Elements are copied from ring buffer order to linear order starting at
    /// index 0 in the destination storage. Source slots remain initialized.
    ///
    /// - Parameters:
    ///   - heapStorage: The destination heap storage.
    ///   - header: The cyclic ring buffer header.
    ...
    public func copy<let N: Int>(
        ...
        _ = unsafe withUnsafePointer(to: _storage) { base in
            unsafe heapStorage.withUnsafeMutablePointerToElements { destination in
                ...
                (.zero..<header.count).forEach { dstIndex in
                    let linearIdx = Index<Element>(Ordinal(index.rawValue.position.rawValue))
                    let source: Pointer<Element>.Immutable = address.pointer(
                        at: linearIdx,
                        stride: Self.slotStride,
                        as: Element.self
                    )
                    unsafe (destination + dstIndex).initialize(to: source.pointee)
                    index += .one
                }
            }
        }
