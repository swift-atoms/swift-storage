...
extension Storage.Inline where Element: ~Copyable {
    ...
    public var `deinitialize`: Property<Storage.Deinitialize, Self>.Inout {
        ...
        _read {
            yield unsafe Property<Storage.Deinitialize, Self>.Inout(self)
        }
        mutating _modify {
...
extension Property.Inout where Base: ~Copyable {
    ...
    public mutating func callAsFunction<
        Element: ~Copyable,
        let count: Int
    >(at slot: Index<Element>.Bounded<count>) where Tag == Storage<Element>.Deinitialize, Base == Storage<Element>.Inline<count> {
    ...
    ) where Tag == Storage<Element>.Deinitialize, Base == Storage<Element>.Inline<count> {
    ...
    /// ```
    @inlinable
    public mutating func all<Element: ~Copyable, let count: Int>()
    where Tag == Storage<Element>.Deinitialize, Base == Storage<Element>.Inline<count> {
        for bitIndex in base.value._slots.ones {
            unsafe base.value._mutablePointer(at: bitIndex.retag(Element.self))
                .deinitialize(count: .one)
        }
        base.value._slots.clear.all()
    }
    ...
    @unsafe
    @inlinable
    public func callAsFunction<Element: ~Copyable, let count: Int>()
    where Tag == Storage<Element>.Deinitialize, Base == Storage<Element>.Inline<count> {
        for bitIndex in base.value._slots.ones {
            unsafe base.value._mutablePointer(at: bitIndex.retag(Element.self))
                .deinitialize(count: .one)
