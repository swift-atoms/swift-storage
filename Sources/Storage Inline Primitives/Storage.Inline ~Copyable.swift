...
extension Storage.Inline where Element: ~Copyable {
    ...
    @unsafe
    @inlinable
    public func pointer(at slot: Index<Element>.Bounded<count>) -> UnsafeMutablePointer<Element> {
    ...
    @inlinable
    @_disfavoredOverload
    public func pointer(at slot: Index<Element>.Bounded<count>) -> UnsafePointer<Element> {
...
extension Storage.Inline where Element: ~Copyable {
    ...
    ///
    /// This is a runtime-accessible view of the compile-time `count` parameter.
    /// Witnesses the `capacity` requirement of `Storage.`Protocol``.
    @inlinable
    public var capacity: Index<Element>.Count {
        // WHY: `count` is a compile-time constant generic parameter, so the Count
        // conversion is total — it cannot exceed Ordinal range for any valid type.
